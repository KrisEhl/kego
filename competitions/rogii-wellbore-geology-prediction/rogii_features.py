"""Sequence-estimator feature pipeline for Rogii TVT prediction.

Faithful port of the public 9.85-ft drift-targeting solution's feature builder
(wellbore-geology-prediction-lightgbm.ipynb / aeroridge / h-blend — all carry
the same inline implementations; utils.py is not public so these are ported
from the notebook cells verbatim, parameterised for our data dir).

Estimators (each individually weak vs constant, but collectively the GBM
reaches ~10 ft):
  - 7-variant Numba beam search (Viterbi-style) through typewell GR
  - particle filters: PF-ANCC (rate model) + PF-Z (z-velocity model)
  - multi-scale NCC with score-weighted ensemble
  - FormationPlaneKNN: weighted plane fit per formation, leave-one-well-out
  - DenseANCCImputer: dense ANCC spatial imputation
  - cross-estimator disagreement, GR offsets/rolls/lags, slope extrapolations

Target is drift = TVT - last_anchor_tvt (post-PS rows only).
"""
...
# 7 beam variants: (beam_size, monotone_cost, emission_scale, smooth_radius, tag)
BEAMS = [
...
def seg_b_well(ktvt, kz, form_col):
    """Segmented b_well: early/mid/late thirds + full prefix + WLS."""
    bv = ktvt + kz - form_col
    n = len(bv)
...
def multi_scale_ncc(kgr, ktvt, hgr, hws=(8, 15, 25), stride=3):
    """Multi-scale NCC. Returns per-scale signals + score-weighted ensemble."""
    out = []
    for hw in hws:
        win = 2 * hw + 1
        nk = len(kgr)
...
class FormationPlaneKNN:
    """Weighted plane fit per formation from k nearest wells (leave-one-well-out)."""
...
class DenseANCCImputer:
    def __init__(self, well_ids, data_dir: Path, spw=DENSE_SPW):
        xs, ys, anccs, wids = [], [], [], []
        for wid in well_ids:
            p = data_dir / f"{wid}__horizontal_well.csv"
            try:
...
def _warmup_numba():
    """Trigger JIT compilation once (avoids per-call overhead in the parallel loop)."""
    md = np.linspace(1, 50, 20, np.float64)
    z = np.zeros(20, np.float64)
    gr = np.full(20, 50.0, np.float64)
    gg = np.linspace(45, 55, 100, np.float64)
    _pf_ancc(md, z, gr, gg, 45.0, 0.1, 20.0, 50.0, 0.0, 8, 0.998, 0.002, 0.005, 0.3, 0.1, 0.001, 0.5)
    _pf_z(
        md, z, gr, gr, gg, gg, 45.0, 0.1, 20.0, 50.0, 0.0, -1.0, 0.0, 0.1, 8, 0.993, 0.005, 0.01, 0.3, 0.2, 0.003, 0.5
    )
...
def build_well(hw_path, tw_path, is_train, FI: FormationPlaneKNN, DI: DenseANCCImputer):
    ...
    for w in [5, 21, 51, 101]:
        rr = gr_s.rolling(w, center=True, min_periods=1)
        rolls[f"grm{w}"] = rr.mean().iloc[ev.index].values.astype(np.float32)
        rolls[f"grs{w}"] = rr.std().fillna(0).iloc[ev.index].values.astype(np.float32)
    ...
    gr_nrg = (
        np.sqrt(np.maximum((gr_s**2).rolling(21, center=True, min_periods=1).mean(), 0.0))
        .iloc[ev.index]
