...
| v14-catboost | CatBoost **GPU** (`task_type=GPU`, depth7, lr0.03, 3000it, od_wait80), same 198-feat suite, `--all-folds` | TBD | Priority-1 family comparison vs xgb (10.62). `--model` dispatch added to `train_seq_feats.py`. Tests whether CatBoost beats xgb on these features + provides ensemble diversity for later. |
