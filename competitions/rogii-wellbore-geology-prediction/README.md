...
| v15-divergence | XGBoost GPU + **v4 Phase-1 estimator-divergence features** (212 cols: 10 pairwise diffs + range/std/min/max across form/ncc/beam/pf/dense drift estimates) | TBD | Priority-2 feature work (ref R10 "+v4"). Pure post-build arithmetic on existing drift cols, no new deps. Debug smoke OK (+14 cols). Divergence-only first for clean attribution; slopes (v16) + DWT-GR (v17, needs pywt) next if it helps. |
