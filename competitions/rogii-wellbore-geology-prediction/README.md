...
- **RE-CORRECTED next-step ladder (2026-05-31, after the post-processing phantom + 2 neutral feature ports):** **(A) ⏳ ensemble @ depth6 regularization** — v27 showed depth6 helps single-XGB −0.047 (3 seeds, all better, above noise); never tested on the ENSEMBLE (v34 was an env-no-op). Now testable via the new `--xgb-depth`/`--cb-depth` CLI flags. **3-seed paired ens-d6 vs ens-d7 (known 10.603), large-N OOF.** → **(B) Optuna-tune XGB+CB reg** (reg_alpha/lambda/mcw/subsample — the ref's actual edge) → **(C) the 3 unported features** (anchor-NCC/gr_xcorr/RowKNN) only if A+B stall (low EV — 2 ports already neutral) → **(D) multi-seed NNLS/Ridge stack LAST.** Post-processing (SG/drift-attenuation) and isolated-feature porting are de-prioritized (phantom / neutral respectively).
