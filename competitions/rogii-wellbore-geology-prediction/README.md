...
- **Direction confirmed for GPU**: script supports `--model ensemble --ensemble-members xgboost,catboost,hgb`. **Cluster-return plan**: (1) GPU full xgb+cb+hgb NNLS multi-seed (the stack, where small gains compound — ref reached 9.85); (2) Optuna-tune (depth, lr, reg) properly; (3) build load-models kernel for fast LB submit; (4) submit best. HGB (+0.04 paired) confirmed worth including; divergence/slopes/regularization all marginal-or-neutral.
