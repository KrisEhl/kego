...
| v31-ensemble-XC | **NNLS ensemble XGB+CatBoost** (both GPU on cluster, 195-feat, no-div/no-kin), 3 seeds | **OOF 10.603 (3-seed mean, std 0.004) — −0.150 vs single XGB** | **CONFIRMED structural win** (priority-4 lever pays off). Per-seed ensemble: 10.598/10.606/10.605 (rock-solid). vs single XGB 10.753, single CatBoost 10.710. So: **CatBoost > XGB by ~0.04 (single)**, and **NNLS blend > best-single by ~0.11**, consistent across all 3 seeds → far above the ~0.05–0.15 noise band. NNLS weights ~XGB 0.48–0.52 / CB 0.55–0.60. Leak-free (NNLS fit on OOF, applied to test); structural (variance reduction) → should transfer to private LB. HGB dropped (28min/fold CPU bottleneck, marginal). (Threading: 3-concurrent OOMs the 62GB node @ ~18GB/job → run ≤2 concurrent.) **Ensemble kernel BUILT (ca1a134) but NOT yet submitted** — audit (3 judges): correctness PASS, deployment CONCERNS (rebuild stale kernel; CatBoost confirmed on Kaggle; ~2-3.5h CPU runtime OK), **statistical CONCERNS: the ensemble+blend STACK is unvalidated — the consensus blend w=0.125 was tuned on single-XGB OOF, but the ensemble has a different base drift (NNLS sum ~1.07 inflates vs blend shrinks → opposing) and pf/beam/dense are already model inputs (double-count risk).** → re-tuning the blend on the *ensemble* OOF (`tune_ensemble_blend.py`, saves the ensemble OOF) before submitting. |
