...
- **RE-CORRECTED next-step ladder (2026-05-31, after the post-processing phantom + 2 neutral feature ports):** **(A) 🔄 ensemble @ depth6 regularization — SUBMITTED (cluster, in flight)** — v27 showed depth6 helps single-XGB −0.047 (3 seeds, all better, above noise); never tested on the ENSEMBLE (v34 was an env-no-op). Now testable via the new `--xgb-depth`/`--cb-depth` CLI flags (smoke-confirmed they forward to the cluster, unlike the env var). **3-seed paired: ens-d6 s1/s2/s3 (f65394/1d1349/a3f7aa) + a d7-s1 baseline cross-check (58eb47, reproducing v31 10.603) — large-N OOF.** Compare d6 mean vs the rock-solid d7 baseline 10.603 (seed std 0.004). → **(B) Optuna-tune XGB+CB reg** (reg_alpha/lambda/mcw/subsample — the ref's actual edge) → **(C) the 3 unported features** (anchor-NCC/gr_xcorr/RowKNN) only if A+B stall (low EV — 2 ports already neutral) → **(D) multi-seed NNLS/Ridge stack LAST.** Post-processing (SG/drift-attenuation) and isolated-feature porting are de-prioritized (phantom / neutral respectively).
