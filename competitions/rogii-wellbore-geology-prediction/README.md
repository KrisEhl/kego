...
| v38-xgblgb | **LGB as a 3rd ensemble member** (ref R11 = LGB+XGB+CB): XGB+LGB NNLS, depth6, LGB 63-leaf/3000 (deployable), 3 seeds, LOCAL/offline (cluster down) | IN FLIGHT — does LGB add diversity? | Tests whether LGB (CPU-fast, leaf-wise, genuinely diverse vs XGB) is a viable member. Compare XGB+LGB NNLS vs known XGB-alone depth6 ~10.60 + XGB+CB 10.52 (same cache). Revisits the v33 LGB kill (that was 255-leaf CPU-slow + the now-corrected "OOF≠LB" framing). If LGB is a good partner → cluster: full XGB+CB+LGB 3-member. Priority-4 (ensemble diversity); the ref's actual edge incl LGB. |
