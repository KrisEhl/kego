...
| v39-ncctw | **Typewell-template NCC** (`ncc_tw_delta`, ref GR_ncc_delta): multi_scale_ncc of eval GR vs typewell GR (±150 of last_tvt). Single-XGB depth6 A/B with vs --no-ncctw, 3 seeds, LOCAL/offline | IN FLIGHT (priority-2 feature) | We had only anchor-template NCC; this adds the typewell variant we lack. **Weak signal expected** (debug target-corr 0.043, std 52 — wide window matches noisily; consistent w/ the NCC-band dead-end), but the ref uses it as a GBM feature in combination. Leak-free (typewell + own GR). A/B on a fresh cache. If neutral → typewell-NCC dead-end (beam already covers typewell-matching); if it helps → refine (per-row window) + ensemble. |
