...
| v24-blend-kernel | **PF-blend only (w=0.10)** on the actual 195-feat model (no divergence), re-tuned via `tune_pp_blend.py` | **10.795 ft (CPU OOF, −0.054)** | **SHIPPED to kernel.** Baseline 10.849 → 10.795. Broad smooth peak w=0.075–0.125 (all within 0.003 of peak) = real signal, not a spike. `d = drift·0.9 + pf_ancc_delta·0.10`; attenuation + savgol DROPPED (inert/overfit per v23 correction). Divergence dropped in `_xy` → the **only** change vs the LB-10.538 anchor is the blend (clean LB attribution). `pf_ancc_delta` is an input (MD/geometry/known-zone GR + type-well log) → no leakage. Params in `outputs/pp_blend_198.json`; OOF in `oof_198.npz`. → submit for 2nd LB anchor. |
| v25-kinematics | **+ trajectory kinematics + apparent-dip physics** (16 feat: incl/azi/dls/build_rate/cos·sin-incl + tvt_dip_full/late + b_dip + plane_dip + azi_delta + apparent_dip_dir), ported from the 8.905 reference | TBD (multi-seed A/B running) | The biggest documented missing lever. Reconstructed faithfully from the reference's usage equations (utils.py is in the author's private dataset — GATE done: kernel pulled, no public dataset). Sign convention pinned to TVT=−Z (`cos(incl)=−dZ/dMD`). **Build validated**: incl median 90.6° (horizontal eval zone ✓), 0% NaN, `tvt_dip_late_d` corr 0.34 w/ target (strong, non-leaking). Leak-free (geometry + anchor TVT_input only; plane-dip via leave-one-well-out KNN). A/B via `--no-kinematics` flag on one cached build (paired seeds vs the 195-feat baseline). Result pending. |

## 🎯 LB ANCHOR (2026-05-30): **public LB 10.538** — v13c kernel (198 feat, no divergence, single xgb, train-in-kernel CPU)

First ground-truth point — **n=1, do not over-interpret** (audit 2026-05-31). OOF 10.62 → LB 10.538 (LB **0.08 better** than OOF). One point cannot establish a slope/"1:1" relationship, and it contradicts the reference ladder where LB runs **0.6–0.95 better** than OOF. So: a denoised CV gain *plausibly* translates, but the OOF→LB offset is unknown — need ≥2–3 LB anchors before trusting any CV→LB extrapolation. Reference best LB = 8.905 (R11 ensemble+tuning), R6 = 9.41 — so ~1.1 ft to reach R6-level. Submission path = code-comp kernel (`-f submission.csv -k <kernel> -v <ver>`), ~80min scoring-env re-run per submit, so submit only multi-seed-validated improvements.

**2nd anchor IN FLIGHT (2026-05-31)**: v24 kernel = anchor pipeline + PF-blend(w=0.10), only change is the blend (OOF −0.054). Expected LB ~10.48 if the blend transfers; the disjoint-half test gives mean +0.055 (min −0.006) so it's unlikely to hurt. This is the 2nd CV↔LB calibration point. Audit verdict: 3 focused judges (pp-correctness PASS, transfer/statistical + provenance both FIXED via blend-only re-tune + div drop).
