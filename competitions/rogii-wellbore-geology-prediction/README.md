...
- **Kernel provenance (audit J8)**: the LB-10.538 kernel was **198-feat (commit `9464a76`, pre-divergence)**; current source builds 212-feat unconditionally → the exact artifact isn't reconstructible (impact ~0 since divergence is neutral, but **add `--no-divergence`-equivalent to the kernel + record git_sha before the next submit**). Also: kernel re-runs are CPU + `xgboost>=2.0` unpinned → expect ±0.15-0.3 ft re-run variance.
