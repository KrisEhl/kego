...
| v40-cluster-resume | **Cluster restored** (2026-06-01): SSH-restarted MLflow server + `kego cluster start` (Ray); `kego sync` copied 18 offline runs (reg-validation/XGB+LGB/ncctw) into MLflow. Resuming cluster batch | IN FLIGHT | Smoke d6-2mem-s1 building the fresh cache (md5 w/ ncc_tw, dropped via --no-ncctw → 195 feat). **Planned: (1) XGB+CB+LGB 3-member (ref R11, priority-4) vs XGB+CB 2-member, depth6, paired — does the weak-but-diverse LGB add? (2) depth6 5-seed confirmation (audit's required fix to ship depth6).** ⚠️ Fresh cache = new pf_ancc (PF unseeded) → compare WITHIN this cache (paired), not vs v36's 10.521. |
