...
| v42-pf-mult | **Variance-reduce pf_ancc via more PF particles** (`--pf-mult`, ROGII_PF_MULT scales ANCC_N/PF_N=600). mult4 d7-2mem vs the seeded mult1 baseline | IN FLIGHT — pfm4 cache built (~40min, 4× particles); mult4-s1 training (fold0 xgb 10.226 vs mult1 10.296 = −0.07, promising); s2 fanned out | Seeded mult1 d7-2mem (4e74197308) = **10.5514** (s2; s1 FAILED) — a MIDDLING pf draw (vs lucky unseeded 10.468 on 885dad2ac0). A cleaner (lower-variance) pf_ancc should beat the middling single draw + be reproducible/stable. mult4 = 4× particles (~2× less variance, ~3× build). Cache-key fix: ROGII_PF_MULT added to the key (_pfmX) so mult variants don't collide. Smoke-verified mult propagates (pf_ancc differs). If mult4 < 10.55 (cleaner pf helps) → tune the level + ship a stable seeded+mult kernel; else pf is tapped. |
