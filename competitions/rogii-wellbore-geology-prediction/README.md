...
- **RE-CORRECTED next-step ladder (2026-05-31, after the post-processing phantom + 2 neutral feature ports):** **(A) 🔄 ensemble @ depth6 regularization — 1st attempt OOM-failed, re-running ≤2 concurrent** — v27 showed depth6 helps single-XGB −0.047 (3 seeds, all better, above noise); never tested on the ENSEMBLE (v34 was an env-no-op). Now testable via the new `--xgb-depth`/`--cb-depth` CLI flags (smoke-confirmed they forward to the cluster, unlike the env var). **3-seed paired d6-vs-d7 on the CURRENT cache (3b5f324fb7).** ⚠️ **FRESH d7 baseline ≠ v31:** d7-s1 (58eb47) ensemble OOF = **10.524** (xgb 10.718 / cb 10.604, NNLS wts 0.395/0.685) — **−0.079 better than the cross-era v31 10.603**, so the ensemble seed/code variance is larger than v31's claimed 0.004; comparing d6 against v31's number would be invalid. → run d7 AND d6 at matched seeds 1/2/3 on this cache (paired). **Partial (2/3 seeds): d7 s1=10.5241 s2=10.5514; d6 s1=10.5055 s2=10.5484 → paired Δ(d6−d7) s1=−0.0186, s2=−0.0030 (mean −0.011, both d6≤d7 but TINY).** Ensemble already regularizes via 2-model averaging → depth6's marginal benefit ≪ single-XGB's −0.047. d7 seed variance ~0.027 (≫ v31's 0.004) — paired delta (controls for seed/folds) is the signal. ⚠️ **2nd OOM mode: GPU VRAM** — d7-s3 (44aa8a) `cudaErrorMemoryAllocation` when 2 default-num_gpus=0.5 jobs packed onto one 3090. **Fix: `--heavy-gpu` (1 full 24GB card/job, no sharing) — BUT only 1 heavy_gpu slot exists** (head's single 3090; 2nd card/worker not heavy_gpu-tagged) → 2nd --heavy-gpu job PENDs (serializes to 1, idles the 2nd GPU). Resolution: d7-s3 (4ed6c6) runs --heavy-gpu (full card 1); cancelled the PENDING heavy d6-s3 (19d983) and resubmitted **default** d6-s3 (75d2af) → lands on the free card 2 → 2 concurrent on SEPARATE cards (no VRAM contention). Closes seed 3. ⚠️ **OOM LESSON (refines the ≤2-concurrent rule):** submitting all 3 d6 (f65394/1d1349/a3f7aa) while the d7-s1 cross-check (58eb47) ran = **4 concurrent ensemble jobs → node OOM at 60/62 GB**; each job loads the full 3.78M-row cache **~14.6 GB at STARTUP** (before the GPU task), so 4× ≈ 58 GB + overhead → Ray killed all 3 d6 (kept the oldest). **Rule: ≤2 concurrent ensemble jobs total (startup load, not just training, counts). Re-run d6 ≤2 at a time after the smoke frees the node.** → **(B) Optuna-tune XGB+CB reg** (reg_alpha/lambda/mcw/subsample — the ref's actual edge) → **(C) the 3 unported features** (anchor-NCC/gr_xcorr/RowKNN) only if A+B stall (low EV — 2 ports already neutral) → **(D) multi-seed NNLS/Ridge stack LAST.** Post-processing (SG/drift-attenuation) and isolated-feature porting are de-prioritized (phantom / neutral respectively).
