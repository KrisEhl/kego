...
| v40-3member | **XGB+CB+LGB 3-member (ref R11) vs XGB+CB 2-member, depth6, paired** (fresh cache 885dad2ac0, LGB 63-leaf via --runtime-env-json) | IN FLIGHT | Cluster restored (MLflow+Ray restarted, 18 offline runs synced). d6-2mem-s1 (smoke): cache built, fold0 xgb 10.295. Fan-out ≤2 concurrent on separate cards (smoke default card1 + d6-3mem-s1 heavy-gpu card2). Remaining: d6-2mem s2/s3, d6-3mem s2/s3. **Does the weak-but-diverse LGB (v38: LGB-alone weakest, NNLS wt ~0.30) add over XGB+CB?** ⚠️ Fresh cache = new pf_ancc (PF unseeded) → compare WITHIN this cache (paired), not vs v36's 10.521. If it helps → ship (audit); else 3-member dead-end → depth6 5-seed next. |
