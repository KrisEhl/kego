...
| v13c-gpu | XGBoost **GPU** (`tree_method=hist, device=cuda`) + vectorised `inf→nan` (np), full 198-feat suite, `--all-folds` 1 job | **10.62 ft (4-fold OOF)** | **PLATEAU BROKEN — full OOF confirmed.** Folds: 10.21/10.61/10.12/11.48 (fold 3 worst, spread 1.4 ft, no leakage-low fold). **15.37→10.62 (−4.75 ft).** GPU @97% util, 17m total (vs ~50m CPU) — both fixes confirmed. 14,151 test preds saved → `outputs/submission_seq_feats.csv`. Sanity: ≈ ref OOF 10.01 (gap = single-xgb vs their tree+blend); not bit-identical to v13b → fresh code. **Next: submit to Kaggle for LB anchor (CV-optimism caveat → LB is ground truth).** |
...
| v18-ensemble | **NNLS blend XGB+CB** on 212 divergence features (one build, both models per fold, scipy.optimize.nnls) | TBD | Reference's dominant lever (R6→R11). `--model ensemble` added to train_seq_feats. Debug smoke OK (blend < both members). Feature eng exhausted (divergence +0.14 > ref full-v4 +0.10; slopes dead-end). |
