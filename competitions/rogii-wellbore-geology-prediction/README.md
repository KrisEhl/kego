...
| v15-divergence | XGBoost GPU + **v4 Phase-1 estimator-divergence features** (212 cols: 10 pairwise diffs + range/std/min/max across form/ncc/beam/pf/dense drift estimates) | **10.48 ft** | **WIN −0.14 vs v13c.** Folds 10.03/10.28/10.35/11.21 (3 of 4 improved; fold 2 +0.23 minor). Pure post-build arithmetic, no new deps. Confirms ref R10 v4 direction → continue ladder: slopes (v16), DWT-GR (v17). New best single model. |
...
First ground-truth point. **OOF 10.62 → LB 10.538** → for our pipeline **LB ≈ OOF within ~0.1** (CV tracks LB ~1:1; no large optimism gap like the reference's). Implication: a *denoised* (multi-seed) CV improvement should translate to LB. Reference best LB = 8.905 (R11 ensemble+tuning), R6 = 9.41 — so ~1.1 ft to reach R6-level. Submission path = code-comp kernel (`-f submission.csv -k <kernel> -v <ver>`), ~80min scoring-env re-run per submit, so submit only multi-seed-validated improvements.
...
- **Direction confirmed for GPU**: the script ALREADY supports `--model ensemble --ensemble-members xgboost,catboost,hgb` → fire the full 3-model NNLS ensemble + Optuna on GPU when cluster returns (ref's ~0.7 lever to 9.85). CPU too slow for catboost/HGB multi-seed at scale. **Cluster-return plan**: (1) GPU full xgb+cb+hgb NNLS multi-seed (expect ~10.45-10.50 OOF, LB ~10.4); (2) Optuna-tune; (3) build load-models kernel for fast LB submit; (4) submit best.
