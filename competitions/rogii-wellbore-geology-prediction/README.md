...
| v41-seed-pf | **Seed the particle filter** (deterministic pf_ancc → reproducible cache rebuilds) — unblocks trustworthy marginal-lever tests | PLANNED | The unseeded PF adds ~0.13 OOF/rebuild, dominating every marginal lever (depth/LGB/feature ports all came up within this noise). Seeding removes the rebuild noise so a real ~0.03 lever could be detected. Until then, marginal tuning is untrustworthy. **Q: does weak-but-diverse LGB add over XGB+CB?** If yes → ship (audit); else dead-end → depth6 5-seed. |
