...
- **RE-CORRECTED next-step ladder (2026-05-31, after the post-processing phantom + 2 neutral feature ports):** **(A) ✅ ensemble @ depth6 — CONFIRMED small win (3-seed paired, all negative).** d6 vs d7 (same cache 3b5f324fb7, paired by seed): s1 −0.0186, s2 −0.0030, s3 −0.0852 → **mean Δ = −0.036 ± 0.044, 3/3 seeds d6<d7.** d6 mean OOF **10.521** vs d7 mean 10.557. Corroborates v27 (single-XGB depth6 −0.047). Real directional win but small (mean within ~1 std); **best ensemble OOF config so far.** Cluster-ops lessons logged (see commits): the env-var depth override is a no-op on cluster → use the new `--xgb-depth`/`--cb-depth` CLI flags; **≤2 concurrent ensemble jobs** (each loads the 3.78M-row cache ~14.6 GB at startup → 4 concurrent OOM'd the 62 GB node); GPU-VRAM OOM if 2 default num_gpus=0.5 jobs share one 3090 → only 1 `heavy_gpu` slot, so run 1 heavy + 1 default on separate cards. → **(B) ⏸ Optuna-tune XGB reg at depth6 — PARTIAL (cluster taken down at 9/15 trials each, 2026-05-31)** (`tune_optuna_reg.py`; XGB-alone 4-fold OOF objective, depth6 fixed; current-default enqueued as baseline trial 1 = 10.6170 at random_state=42). **Best so far: worker A (8a7b19) trial-5 = 10.5816** → `{reg_alpha 0.0054, reg_lambda 0.114, min_child_weight 32, subsample 0.773, colsample_bytree 0.646}` — **−0.035 vs the default-reg baseline (clean paired Δ, same random_state=42).** Worker B (2ebcb2) best 10.6081 (−0.009). Low-reg_lambda/higher-mcw/lower-colsample region wins. (Cluster-smoke caught+fixed an argparse prefix-match: kego `--fold 0`→`--folds 0` crash; allow_abbrev=False.) **NEXT (resume when cluster returns):** add reg overrides → validate this reg in the ENSEMBLE (3-seed paired vs d6 10.521); meanwhile validate single-XGB reg locally (multi-seed) to confirm the −0.035 is seed-robust. Best reg → ensemble → audit-run → ship. → **(C) the 3 unported features** (anchor-NCC/gr_xcorr/RowKNN) only if A+B stall (low EV — 2 ports already neutral) → **(D) multi-seed NNLS/Ridge stack LAST.** Post-processing (SG/drift-attenuation) and isolated-feature porting are de-prioritized (phantom / neutral respectively).
