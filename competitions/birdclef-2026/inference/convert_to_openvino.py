"""Convert soundscape-v7 PyTorch checkpoints to OpenVINO IR format."""
...
OUTPUTS_DIR = Path("competitions/birdclef-2026/outputs")
OUT_DIR = Path("/tmp/birdclef-v7-openvino")
OUT_DIR.mkdir(exist_ok=True)

# Input shape: (B, 3, n_mels, time_frames)
# n_mels=224, hop=512, 5s @ 32kHz → 32000*5//512 + 1 = 313 frames
# Use dynamic batch + fixed spatial dims
EXAMPLE_INPUT = torch.zeros(1, 3, 224, 313)

ckpt_paths = sorted(OUTPUTS_DIR.glob("soundscape-v7_fold*.pt"))
if not ckpt_paths:
    print("ERROR: no soundscape-v7 checkpoints found in outputs/")
    sys.exit(1)
...
for ckpt_path in ckpt_paths:
    ...
    with torch.no_grad():
        out = model(example)
    print(f"  PyTorch output: {out.shape}, range [{out.min():.3f}, {out.max():.3f}]")

    # Convert to OpenVINO
    ov_model = ov.convert_model(model, example_input=example)

    # Set dynamic batch dimension
    shapes = {ov_model.input(0): [-1, 3, n_mels, time_frames]}
    ...
    print(f"  Saved: {out_xml} + {stem}.bin")

    # Verify OpenVINO output matches PyTorch
    core = ov.Core()
...
meta = {
    "title": "BirdCLEF 2026 Soundscape-v7 OpenVINO",
    "id": "aldisued/birdclef2026-soundscape-v7-openvino",
...
for p in sorted(OUT_DIR.iterdir()):
    ...
    print(f"  {p.name:45s} {size_mb:.1f} MB")
