...
def get_phase(i, planet_id, n_steps, dataset, adc_info):
    f_signal = pd.read_parquet(os.environ["FOLDER_COMPETITION"] + f"{dataset}/{planet_id}/FGS1_signal.parquet")
...
if __name__ == "__main__":
    ...
    dataset = "train"
    train_adc_info = pd.read_csv(os.environ["FOLDER_COMPETITION"] + "train_adc_info.csv", index_col="planet_id")
    ...
    for n_steps in [100, 400, 1000, 2000, 8000][::-1]:
        ...
        phases.append(phase)
        # phases = result.get()
        df = pd.DataFrame(phases, columns=[f"phase_{i}" for i in range(len(phases[0]))], index=indices)
