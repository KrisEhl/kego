"""kego cancel — cancel Ray jobs for a given kego experiment ID."""
...
def _ray_delete(base: str, submission_id: str) -> None:
    """Send DELETE to the Ray Jobs API to stop a job."""
...
def _cancel(args: argparse.Namespace, extra_args: list[str]) -> int:
    ...
    for run in runs:
        ...
        try:
            _ray_delete(base, submission_id)
