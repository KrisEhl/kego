...
def plot_grid_plot(
    ...
    if data_as_dict is None:
        if xs is None or ys is None:
            raise ValueError(f"{data_as_dict=} is not set. Please specify both {xs=} and {ys=} instead")
    ...
    figure, axes_grid, _axes_colorbar = create_axes_grid(n_columns=n_columns, n_rows=n_rows)

    for i_plot, (label, data) in enumerate(data_as_dict.items()):
        if style == "colormesh":
            axes = np.ndarray.flatten(axes_grid)[i_plot]
            xx, yy = np.meshgrid(np.arange(np.shape(data)[1]), np.arange(np.shape(data)[0]))
