...
class TestEvalMultiseed:
    ...
    def test_useful_feature_scores_higher_than_random(self, classification_data, shared_kwargs):
        X_train, y_train, X_holdout, y_holdout = classification_data
...
class TestDropOneAblation:
    ...
    def test_removing_useful_hurts(self, classification_data, shared_kwargs):
        ...
        useful_entry = next(entry for entry in result.feature_results if entry["feature"] == "useful")
        # Removing useful should hurt -> negative delta
        assert useful_entry["delta"] < 0

    def test_selected_features_exclude_harmful(self, classification_data, shared_kwargs):
        X_train, y_train, X_holdout, y_holdout = classification_data
        result = drop_one_ablation(
            X_train,
            y_train,
...
class TestForwardSelection:
    ...
    def test_useful_first_selects_at_least_useful(self, classification_data, shared_kwargs):
        X_train, y_train, X_holdout, y_holdout = classification_data
        result = forward_selection(
            X_train,
            y_train,
    ...
    def test_score_improves_with_useful_feature(self, classification_data, shared_kwargs):
        X_train, y_train, X_holdout, y_holdout = classification_data
        result = forward_selection(
            X_train,
            y_train,
    ...
    def test_selected_score_matches_best_prefix(self, classification_data, shared_kwargs):
        X_train, y_train, X_holdout, y_holdout = classification_data
        result = forward_selection(
            X_train,
            y_train,
...
class TestGreedyAddOneScreening:
    ...
    def test_useful_candidate_has_positive_delta(self, classification_data, shared_kwargs):
        X_train, y_train, X_holdout, y_holdout = classification_data
        result = greedy_add_one_screening(
    ...
    def test_selected_includes_helpful_candidates(self, classification_data, shared_kwargs):
        X_train, y_train, X_holdout, y_holdout = classification_data
        result = greedy_add_one_screening(
            baseline_features=["random"],
            candidate_features=["useful"],
    ...
    def test_accepts_precomputed_baseline_score(self, classification_data, shared_kwargs):
        X_train, y_train, X_holdout, y_holdout = classification_data
        result = greedy_add_one_screening(
            baseline_features=["useful"],
            candidate_features=["random"],
