...
import mlframework.plotting.histogram_2d_utils
...
def plot_histogram_2d(
    ...
    colormap: str = mlframework.constants.DEFAULT_COLORMAP,
    ...
    axes_colorbar: mlframework.constants.TYPE_MATPLOTLIB_AXES | None = None,
) -> dict:
    """
    ...
    axes_style = mlframework.plotting.histogram_2d_utils._resolve_axes_style(axes)
    ...
    (
        ...
        spacing_x,
        spacing_y,
    ) = mlframework.plotting.histogram_2d_utils._resolve_defaults(
        facet_column,
        facet_row,
        marginal_x,
        marginal_y,
    ...
    mlframework.plotting.histogram_2d_utils._check_parameter_consistency(
    ...
    (
        ...
        facet_column_title,
    ) = mlframework.plotting.histogram_2d_utils._prepare_facet(df, facet_column)
    (
        row_masks,
        n_facet_row_values,
        facet_row_title,
    ) = mlframework.plotting.histogram_2d_utils._prepare_facet(df, facet_row)

    (
        n_columns,
        n_rows,
    ) = mlframework.plotting.histogram_2d_utils._set_number_rows_and_columns(
    ...
    (
        xaxis_labels,
        yaxis_labels,
    ) = mlframework.plotting.histogram_2d_utils._set_axis_labels(
        n_columns,
        n_rows,
        facet_show_label_x,
        facet_show_label_y,
    ...
    masks = mlframework.plotting.histogram_2d_utils._set_masks(
        column_masks, row_masks, n_columns, n_rows
    )

    titles = mlframework.plotting.histogram_2d_utils._determine_axes_titles(
        facet_column,
        facet_row,
        title,
        facet_column_title,
    ...
    (
        ...
        colorbar_off,
    ) = mlframework.plotting.histogram_2d_utils._prepare_axes_grid_creation(
    ...
    if axes_style == "axes_grid":
        (
            ...
            axes_grid,
            axes_colorbar_grid,
        ) = mlframework.plotting.utils_plotting.create_axes_grid(
            n_columns=n_columns,
            n_rows=n_rows,
...
def _plot_histogram_2d(
    x_key_or_values: np.ndarray | str,
    y_key_or_values: np.ndarray | str,
    axes: mlframework.constants.TYPE_MATPLOTLIB_AXES,
    axes_colorbar: mlframework.constants.TYPE_MATPLOTLIB_AXES,
    xlim: tuple[float | None, float | None] | None = None,
    ylim: tuple[float | None, float | None] | None = None,
    df: pd.DataFrame | None = None,
    bin_edges: tuple[np.ndarray, np.ndarray] | None = None,
    log: str | tuple[str, str] = "false",
    fig: mlframework.constants.TYPE_MATPLOTLIB_FIGURES | None = None,
    n_bins: int | tuple[int, int] = 60,
    n_bins_linear: int = 10,
    norm: str | None = None,
    norm_symlog_linear_threshold: float | None = None,
    ...
    convert_zeros_to_nan: bool = True,
) -> tuple[mlframework.constants.TYPE_MATPLOTLIB_AXES, np.ndarray]:
    """
    Plots 2d histogram
    ...
    mlframework.checks.validate_array(bin_edges_x, name="bin_edges_x")  # type: ignore
    mlframework.checks.validate_array(bin_edges_y, name="bin_edges_y")  # type: ignore

    norm_object = mlframework.plotting.utils_plotting.get_norm(
        norm,
        vmin=vmin,
        vmax=vmax,
        norm_symlog_linear_threshold=norm_symlog_linear_threshold,
    ...
    plot = mlframework.plotting.colormesh._plot_colormesh(
        axes,
        X,
        Y,
        H_plot,
    ...
    if annotate:
        mlframework.plotting.utils_plotting.annotate_values(
            H=H_plot,
            axes=axes,
    ...
    mlframework.plotting.utils_plotting.plot_colorbar(
    ...
    mlframework.plotting.axes_utils.set_x_lim(axes, xlim)
    mlframework.plotting.axes_utils.set_y_lim(axes, ylim)

    mlframework.plotting.utils_plotting.set_title(axes, title, font_size=font_size)
    ...
    if marginal_x == "histogram" and axes_marginal is not None:
        ...
        if not marginal_x_show_xticks:
            mlframework.plotting.utils_plotting.remove_tick_labels(
                axes_marginal[0][0], "x"
            )
...
def _get_xy_values(
    ...
    mlframework.checks.validate_array(x)
    y = np.ndarray.flatten(y)  # type: ignore
    mlframework.checks.validate_array(y)
    if x.shape != y.shape:
        raise ValueError(
...
def plot_histogram(
    ...
    axes: mlframework.constants.TYPE_MATPLOTLIB_AXES | None = None,
    filename: str | pathlib.Path | None = None,
    font_size: float = mlframework.constants.DEFAULT_FONTSIZE_LARGE,
    vertical: bool = False,
    alpha: float = 1,
    color: str | None = None,
    ...
    if tight_layout:
        figure.tight_layout()
    mlframework.plotting.utils_plotting.set_title(axes, title, font_size=font_size)
    mlframework.plotting.figures.save_figure(figure, filename)
    return axes
